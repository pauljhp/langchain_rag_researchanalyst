...
class SECExtractor:
    def __init__(
        ...
        self.company = os.environ.get("SEC_API_ORGANIZATION")
        self.email = os.environ.get("SEC_API_EMAIL")
        if self.company is None: self.company = "Unstructured Extraction"
        if self.email is None: self.email = "unstructured_extraction@io.ai"
    ...
    def get_text_from_url(self, url: str):
        ...
        text = self.get_filing(
            url, company=self.company, email=self.email
        )
